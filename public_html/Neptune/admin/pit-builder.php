...
if($_SERVER['REQUEST_METHOD']==='POST'){
    ...
    try{
        ...
        if(!is_array($raw))throw new RuntimeException('Invalid pit form configuration.');
        ...
        $pitJson=$json;
        $preJson=$draft['pre_scout_config_json'];
        $checksum=neptune_revision_checksum($matchJson,$pitJson,$preJson,$draft['field_image_path']);
        $s=$pdo->prepare("UPDATE game_revisions SET pit_config_json=?,checksum=?,updated_at=CURRENT_TIMESTAMP WHERE id=? AND game_id=? AND organization_id=? AND status='draft'");
        $s->execute([$json,$checksum,(int)$draft['id'],$gameId,$org]);
        $msg='Pit form saved to Draft Revision '.(int)$draft['revision_number'].'. Publish the revision in Game Builder when all configuration is ready.';
        $draftRevision=neptune_revision_by_id($pdo,(int)$draft['id']);
        $viewRevision=$draftRevision;
    }catch(Throwable $e){$error=$e->getMessage();}
}

$config=$viewRevision?json_decode((string)($viewRevision['pit_config_json']??''),true):[];
$questions=is_array($config['questions']??null)?$config['questions']:[];

$pageTitle='Pit Form Builder';
$moduleName='VULCAN';
...
<section class="module-page builder-page">
  <header class="builder-page-header">
    <div>
      ...
      <p>Add game-specific pit questions. Neptune automatically includes the standard robot, autonomous, endgame, strategy, and reliability questions.</p>
    </div>
    <a class="btn secondary" href="<?=e(base_url('pit/index.php'))?>"><i class="fa-solid fa-clipboard-list"></i> Open Pit Scouting</a>
  </header>
  ...
  <div class="card builder-card">
    <div class="builder-card-head">
      <div><h2>Game-Specific Questions</h2><p>Codes auto-fill from the question name and are made unique with _b1, _b2, etc.</p></div>
      <div class="toolbar" style="margin:0"><button type="button" id="addQuestion" <?=$canEdit?'':'disabled'?>><i class="fa-solid fa-plus"></i> Add Question</button></div>
    </div>
    <form method="post" id="pitBuilderForm">
      ...
      <div class="builder-save-row"><button class="good"><i class="fa-solid fa-floppy-disk"></i> Save Pit Draft</button></div>
      </fieldset>
    </form>
  </div>
