...
// Read prior-event OPR from Neptune's persisted AUGUR OPR table. The table is
// built from the same archived qualification samples as Public EPA, so this
// page remains fully local/offline and does not recalculate OPR on page load.
$currentStart=(string)($event['start_date']??'');
$localOprContext=neptune_prescout_opr_context(
    $pdo,$org,(int)$event['game_id'],$eventId,(int)$event['season_year'],
    (string)($event['tba_event_key']??''),$currentStart,[$team]
);
...
$latestEvent=$localOpr['latest_event']??($oprHistory?end($oprHistory):null);
$previousEvent=$localOpr['previous_event']??(count($oprHistory)>1?$oprHistory[count($oprHistory)-2]:null);
...
$oprTotal=$localOpr['season_avg_opr']??($localOpr['season_opr']??($cachedIntel['season_opr']??null));
if(!is_numeric($oprTotal)){
    // Compatibility for older profiles: average available event OPR values.
    // Traditional event OPR values must never be added together.
...
<section class="card prescout-performance-card">
  ...
  <div class="toolbar" style="justify-content:space-between;align-items:flex-start"><div><h2 style="margin-bottom:4px">Season Performance</h2><div class="muted">Public EPA and event OPR are read from Neptune's local AUGUR databases. Season Avg OPR is the qualification-match-weighted average of prior event OPR values, so no OPR Internet request is required.</div></div><div class="prescout-source-links"><a href="https://www.thebluealliance.com/team/<?=$team?>/<?=e($event['season_year'])?>" target="_blank" rel="noopener">Powered by The Blue Alliance</a><a href="<?=e(base_url('analytics/augur-ratings.php?year='.(int)$event['season_year']))?>">Public EPA in Neptune</a></div></div>
  <div class="prescout-performance-grid">
    ...
    <div><span>Most Recent Event OPR</span><b title="<?=e((string)(is_array($latestEvent)?($latestEvent['event_name']??'Most recent prior event'):'Most recent prior event'))?>"><?=e(neptune_prescout_num($opr1,2))?></b></div>
    <div><span>Previous Event OPR</span><b title="<?=e((string)(is_array($previousEvent)?($previousEvent['event_name']??'Previous prior event'):'Previous prior event'))?>"><?=e(neptune_prescout_num($opr2,2))?></b></div>
    <div><span>Season Avg OPR</span><b><?=e(neptune_prescout_num($oprTotal,2))?></b></div>
