...
if($team>0){
    ...
    // Statbotics is season-specific. Only fetch the selected season so Robot
    // Lookup remains fast even if the external service is temporarily slow.
    $statYears=[(int)date('Y')];
    foreach($seasonProfiles as $profile){
        $y=(int)($profile['season_year']??0);
        if($y>0) $statYears[]=$y;
    }
    $statYears=array_values(array_unique(array_filter(
        $statYears,
        static fn($y)=>$y>=1992 && $y<=((int)date('Y')+1)
    )));
    rsort($statYears);

    $requestedStatYear=(int)($_GET['stat_year']??0);
    $statYear=($requestedStatYear>0 && in_array($requestedStatYear,$statYears,true))
        ? $requestedStatYear
        : ($statYears[0]??(int)date('Y'));

    if(function_exists('statbotics_get')){
        try{
            $candidate=statbotics_get("team_year/{$team}/{$statYear}",1800);
            if(is_array($candidate) && empty($candidate['__neptune_error'])){
                $statbotics=$candidate;
            }
        } catch(Throwable $ex){
            $statboticsError=$ex->getMessage();
        }
    } else {
        $statboticsError='Statbotics integration is not installed on this Neptune server.';
    }
    ...
    $s=$pdo->prepare(
        "SELECT ps.*,e.name event_name,e.start_date,g.name game_name,g.season_year
    ...
    $s=$pdo->prepare(
        "SELECT p.*,e.name event_name,e.start_date,g.name game_name,g.season_year
         FROM pre_scouting p
  ...
  <?php if($team>0):?>
    ...
    <div class="card" style="margin-top:16px">
      <div class="robot-lookup-stat-head">
        <div>
          <div class="module-eyebrow"><span>STATBOTICS</span><small>Season Performance Model</small></div>
          <h2 style="margin:0">Statbotics · <?=e($statYear)?></h2>
          <div class="muted">External performance estimates shown separately from Neptune's own scouting observations.</div>
        </div>
        <?php if(count($statYears)>1):?>
          <form method="get" class="toolbar" style="margin:0">
            ...
            <label style="margin:0">
              ...
              <select name="stat_year" onchange="this.form.submit()">
                <?php foreach($statYears as $y):?>
                  <option value="<?=e($y)?>" <?=$y===$statYear?'selected':''?>><?=e($y)?></option>
                <?php endforeach;?>
              </select>
            </label>
          </form>
      ...
      <?php if($statbotics):
        $epa=is_array($statbotics['epa']??null)?$statbotics['epa']:[];
        $breakdown=is_array($epa['breakdown']??null)?$epa['breakdown']:[];
        $stats=is_array($epa['stats']??null)?$epa['stats']:[];
        $ranks=is_array($epa['ranks']??null)?$epa['ranks']:[];
        $record=is_array($statbotics['record']??null)?$statbotics['record']:[];
        $overall=is_array($ranks['total']??null)?$ranks['total']:[];
        $stateRank=is_array($ranks['state']??null)?$ranks['state']:[];
        $districtRank=is_array($ranks['district']??null)?$ranks['district']:[];
        $epaTotal=$breakdown['total_points']??($epa['total_points']??null);
      ...
      ?>
        <?php if(!empty($statbotics['__neptune_stale'])):?>
          <div class="notice" style="margin-top:12px"><i class="fa-solid fa-clock-rotate-left"></i> Statbotics is temporarily unavailable; Neptune is showing the most recent successful cached response.</div>
        <?php endif;?>

        <div class="robot-lookup-kpis">
          <div class="robot-lookup-kpi"><b><?=rl_num($epaTotal,1)?></b><span class="muted">Total EPA</span></div>
        ...
        <div class="robot-lookup-kpis">
          <div class="robot-lookup-kpi">
            <b><?=e(($record['wins']??0).'-'.($record['losses']??0).'-'.($record['ties']??0))?></b>
            <span class="muted">Record</span>
          </div>
          <div class="robot-lookup-kpi">
            <b><?=$winrate!==null?number_format((float)$winrate*100,1).'%':'—'?></b>
            <span class="muted">Win rate</span>
          </div>
          <div class="robot-lookup-kpi"><b><?=rl_num($stats['start']??null,1)?></b><span class="muted">Starting EPA</span></div>
          <div class="robot-lookup-kpi"><b><?=rl_num($stats['max']??null,1)?></b><span class="muted">Max EPA</span></div>
        </div>

        <div class="robot-lookup-stat-ranks">
          <div class="robot-lookup-stat-rank">
            <b><?=isset($overall['rank'])?'#'.e($overall['rank']):'—'?></b>
            <span>Overall rank</span>
            <?php if(isset($overall['percentile'])):?><small><?=rl_percentile($overall['percentile'])?> percentile · <?=e($overall['team_count']??'—')?> teams</small><?php endif;?>
          </div>
          <div class="robot-lookup-stat-rank">
            <b><?=isset($stateRank['rank'])?'#'.e($stateRank['rank']):'—'?></b>
            <span>State rank</span>
            <?php if(isset($stateRank['percentile'])):?><small><?=rl_percentile($stateRank['percentile'])?> percentile · <?=e($stateRank['team_count']??'—')?> teams</small><?php endif;?>
          </div>
          <div class="robot-lookup-stat-rank">
            <b><?=isset($districtRank['rank'])?'#'.e($districtRank['rank']):'—'?></b>
            <span>District rank</span>
            <?php if(isset($districtRank['percentile'])):?><small><?=rl_percentile($districtRank['percentile'])?> percentile · <?=e($districtRank['team_count']??'—')?> teams</small><?php endif;?>
          </div>
        </div>

        <?php
          $extra=[];
          $skip=['total_points','auto_points','teleop_points','endgame_points'];
          foreach($breakdown as $k=>$v){
              if(in_array($k,$skip,true)) continue;
              if(!is_scalar($v) || $v===null || $v==='') continue;
              $extra[$k]=$v;
          }
        ?>
        <?php if($extra):?>
          <details class="robot-lookup-stat-details" style="margin-top:12px">
            <summary>Full game-specific Statbotics breakdown</summary>
            <div class="robot-lookup-fields" style="margin-top:8px">
              <?php foreach($extra as $k=>$v):?>
                <div class="robot-lookup-field">
                  <b><?=e(rl_label((string)$k))?></b>
                  <span><?=is_numeric($v)?rl_num($v,2):e((string)$v)?></span>
                </div>
              <?php endforeach;?>
            </div>
          </details>
        ...
        <div class="robot-lookup-tba-note">Source: Statbotics. EPA and rankings are model-based estimates; Neptune scouting data below remains your team's observed match data.</div>
      <?php else:?>
        <div class="notice" style="margin-top:12px">
          <i class="fa-solid fa-chart-line"></i>
          No Statbotics data is currently available for team #<?=e($team)?> in <?=e($statYear)?>.
          <?php if($statboticsError!==''):?><span class="muted" style="display:block;margin-top:4px"><?=e($statboticsError)?></span><?php endif;?>
        </div>
      <?php endif;?>
    </div>
          ...
          ?>
            <div class="robot-lookup-event">
              ...
              <div class="robot-lookup-fields">
                <?php foreach($data as $k=>$v): if(!rl_has_value($v)) continue;?>
          ...
          <div class="robot-lookup-event">
            ...
            <?php if($stat):?>
              <div class="robot-lookup-kpis">
                ...
                <div class="robot-lookup-kpi"><b><?=isset($stat['epa'])?rl_num($stat['epa'],1):'—'?></b><span class="muted">EPA</span></div>
