...
<section class="module-page command-page">
  ...
  <div class="card">
    ...
    <form method="post" class="augur-maint-actions" id="augurRunForm">
      ...
      <button type="submit" id="augurRunButton" <?=$running?'disabled':''?>><i class="fa-solid fa-play"></i> <span id="augurRunButtonText"><?=$running?'Job Running':'Run Update'?></span></button>
    </form>
...
(() => {
  ...
  const runButton = document.getElementById('augurRunButton');
  const runButtonText = document.getElementById('augurRunButtonText');
  ...
  async function refreshJob() {
    try {
      ...
      if (runButton) runButton.disabled = !!d.running;
      if (runButtonText) runButtonText.textContent = d.running ? 'Job Running' : 'Run Update';
