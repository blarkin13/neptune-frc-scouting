...
$pageTitle='Home';include dirname(__DIR__).'/partials_header.php';
?>
<section class="card home-hero">
  <div><img class="home-hero-logo" src="<?=e(base_url('images/logo.png'))?>" alt="Neptune"><h1>Welcome to Neptune</h1><p class="muted">FRC scouting, live match command, pit intelligence, and event analytics in one platform.</p></div>
  <div class="status-panel">
    <?php if($event):?>
      ...
      <div class="home-event-metrics"><div><b><?=e($event['team_count'])?></b><span>Teams</span></div><div><b><?=e($event['pit_complete'])?></b><span>Pit complete</span></div><div><b><?=e($event['match_count'])?></b><span>Matches</span></div></div>
...
<section class="dashboard-intro-grid" aria-label="About and setup">
  <article class="card platform-about">
    ...
    <h2>Built for the full FRC scouting workflow</h2>
    <p>Neptune brings event setup, pit scouting, live match scouting, robot intelligence, analytics, and team-controlled data sharing into one platform. Game actions and pit questions are configurable, so Neptune can move from one FRC season to the next without rebuilding the scouting system.</p>
    <div class="platform-feature-row">
      <span><i class="fa-solid fa-cloud-arrow-down"></i> TBA event data</span>
      <span><i class="fa-solid fa-clipboard-list"></i> Pit scouting</span>
      <span><i class="fa-solid fa-crosshairs"></i> Live scouting</span>
      <span><i class="fa-solid fa-chart-line"></i> Robot analytics</span>
    </div>
  ...
  <article class="card platform-setup">
    ...
    <div class="setup-methods">
      <div>
        ...
        <div><b>Hosted</b><span>Upload Neptune to your web server, create the MySQL database, run the schema/upgrades, configure <code>neptune_secure/config.php</code>, and add your TBA API key.</span></div>
      </div>
      <div>
...
<div class="home-links-heading">
  <div class="section-kicker"><i class="fa-solid fa-bolt"></i> QUICK ACCESS</div>
  <h2>Where do you want to go?</h2>
</div>
<div class="quick-grid">
  <a class="quick-link" href="<?=e(base_url('pit/index.php'.($event?'?event_id='.$event['id']:'')))?>"><span class="quick-icon"><i class="fa-solid fa-clipboard-list"></i></span><span><b>Pit Scouting</b><span class="muted">Scout the event roster before matches begin.</span></span></a>
  <a class="quick-link" href="<?=e(base_url('scout/index.php'))?>"><span class="quick-icon"><i class="fa-solid fa-crosshairs"></i></span><span><b>Neptune Agent</b><span class="muted">Open the live match scouting interface.</span></span></a>
  <a class="quick-link" href="<?=e(base_url('analytics/robots.php'))?>"><span class="quick-icon"><i class="fa-solid fa-robot"></i></span><span><b>Robot Analytics</b><span class="muted">Compare every robot at the event.</span></span></a>
  <a class="quick-link" href="<?=e(base_url('analytics/team-display.php'))?>"><span class="quick-icon"><i class="fa-solid fa-tv"></i></span><span><b>Pit Match Board</b><span class="muted">Past and future matches with robot stats.</span></span></a>
  <?php if(in_array($u['role'],['owner','admin','strategy'],true)):?>
    <a class="quick-link" href="<?=e(base_url('admin/events.php'))?>"><span class="quick-icon"><i class="fa-solid fa-calendar-days"></i></span><span><b>Event Setup</b><span class="muted">Create the event, load its roster, and open pit scouting.</span></span></a>
    <a class="quick-link" href="<?=e(base_url('admin/match-control.php'))?>"><span class="quick-icon"><i class="fa-solid fa-tower-broadcast"></i></span><span><b>Match Control</b><span class="muted">Ready, start, pause, replay, and monitor matches.</span></span></a>
    <a class="quick-link" href="<?=e(base_url('admin/tba-sync.php'))?>"><span class="quick-icon"><i class="fa-solid fa-cloud-arrow-down"></i></span><span><b>TBA Event Sync</b><span class="muted">Refresh the roster and add the schedule when it appears.</span></span></a>
  <?php endif;?>
</div>
  ...
  <div class="dashboard-dialog-shell">
    ...
    <div class="dashboard-dialog-body">
      ...
      <div class="howto-steps">
        <div><span>1</span><div><b>Build the game</b><p>Configure the season's match actions, button layout, scoring values, timing, and pit questions. Save the game definition for reuse at every event that season.</p></div></div>
        <div><span>2</span><div><b>Open the event</b><p>Create the event before matches begin. Load the team roster manually or from The Blue Alliance and make it Neptune's Current Event.</p></div></div>
        <div><span>3</span><div><b>Pit scout</b><p>Scouts work through the event roster, recording robot capabilities, autonomous options, endgame abilities, reliability notes, and photos.</p></div></div>
        <div><span>4</span><div><b>Sync the schedule</b><p>When FIRST publishes the schedule, refresh from The Blue Alliance. Neptune fills every match with the official Red and Blue field stations.</p></div></div>
        <div><span>5</span><div><b>Run live scouting</b><p>Command makes the next match ready and starts it. Agents select their assigned field station and record actions in real time while Neptune keeps the match timer synchronized.</p></div></div>
        <div><span>6</span><div><b>Use the intelligence</b><p>Review Robot Cards, the Pit Match Board, individual robot details, past match data, cycle times, success rates, defense, pit information, and shared scouting data.</p></div></div>
      </div>
      ...
      <div class="setup-guide-grid">
        <section>
          ...
          <ol>
            ...
            <li>Create the Neptune database and import the base schema plus each numbered upgrade in order.</li>
        ...
        <section>
          ...
          <ol>
            ...
            <li>Create a local Neptune database and import the same schema/upgrades.</li>
