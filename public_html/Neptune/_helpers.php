 ...
 *
 * OPR 1 / OPR 2 are the first two prior events (chronological) for which the
 * team has enough local qualification data. season_opr is one least-squares
 * solve across every prior qualification match in the same game/season.
 */
function neptune_prescout_local_opr_context(PDO $pdo,int $organizationId,int $gameId,int $currentEventId,?string $currentStart,array $teamFilter=[]): array {
    ...
    $eventMaps=[];$allRows=[];$eventMeta=[];$totalAllianceRows=0;
    ...
    $season=neptune_prescout_opr_solve($allRows);

    $filter=array_values(array_unique(array_filter(array_map('intval',$teamFilter),static fn($n)=>$n>0)));
    if(!$filter){
        $filter=array_map('intval',array_keys((array)($season['ratings']??[])));
    ...
    foreach($filter as $team){
        ...
        $teams[$team]=[
            'events'=>$history,
            'opr1'=>$history[0]['opr']??null,
            'opr2'=>$history[1]['opr']??null,
            'season_opr'=>isset($season['ratings'][$team])?(float)$season['ratings'][$team]:null,
            'matches_used'=>(int)($season['matches'][$team]??0),
            'source'=>'local_opr',
        ];
    }
