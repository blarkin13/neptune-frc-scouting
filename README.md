...
Installs Apache, PHP, MariaDB, and required PHP extensions.

Creates the Neptune MariaDB database and application account.

Generates a random local database password.

Imports the authoritative clean-install schema from sql/install_schema.sql.

Deploys the application and protected configuration.

Configures the Apache site.

Enables required Apache modules.

Validates PHP syntax.

Verifies the required database tables against sql/EXPECTED_TABLES.txt.

Runs an installation health check.
...
URL prefix when Neptune is not the site root
...
Back up the application/configuration.

Apply the update away from competition use.

Run Neptune System Check and installation/application validation.

Confirm scouting and analytics workflows before declaring the build event-ready.
...
Never commit real database passwords, API keys, private configuration, database dumps, or backup files containing secrets.
...
Create/select the season's game in Game Builder.

Create the event and make it the Current Event.

Import or paste the event roster and begin Pit Scouting.

When TBA publishes the schedule, use TBA Sync to import/refresh it.

Use Match Control to make the next match Ready and start it.

Scouts select their assigned field station and record actions.

Use Live Monitor to watch activity and correct erroneous actions.

Review Robot Analytics, Robot Intelligence, and the Pit Match Board.
